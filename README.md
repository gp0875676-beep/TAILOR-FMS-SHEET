...
Two Render services in `render.yaml`:
- **worker** (`fms-telegram-bot`) — the actual bot, long-polling, no HTTP port.
- **web** (`fms-health`) — optional `/health` endpoint if you want Render's
  health checks or an uptime monitor to hit something.

Both share the same PostgreSQL database (`fms-db` in `render.yaml`), which is
